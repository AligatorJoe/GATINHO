...
function DIO_GAME(version, gm, DATA, time_a) {
    var MutationObserver = uw.MutationObserver || window.MutationObserver,
        ...
        dio_sprite = "https://i.imgur.com/cILbyDs.png"; // http://abload.de/img/dio_spritejmqxp.png, http://img1.myimg.de/DIOSPRITEe9708.png -> Forbidden!?
    ...
    var LANG = {
        de: {
            ...
            labels: {
                ...
                WaUp: "si nivel muralla ?25 :",
				Rev2: "image Chama R2",
                Def1: "Defesa Terre",
				Bir1: "Defesa Birremes",
				OC: "OC",
                R2: "R2:",
                R22: "Hora R2",
                // WW
        ...
        ru: {
            ...
            options: {
                ...
                tsk: ["???????", "?????????? ?????? ???????? ? ???????????? ???? ?????????? ??????? ??? ????? ? ????"],
                // cha: ["???", '?????? ???? ???? ?? irc-???'],
                bbc: ["????? ???????", "????????? ?????? ??? ??????? ? ????????? ?????? ? ??????"], // Beschreibung passt nicht ganz
                com: ["????????? ??????", "????????? ???? ????????? ??????"],
                tic: ["???? ???????", "?????? ????? ???????? ?????? ??? ?????????? ???? (?????????????? ???????????)", "?????????????? ?????? ???????? ??? ??????? ??????"], // ?
                til: ["?????? ?????", "????????? ?????? ????????? ? ?????? ??????"], // ?
                tim: ["?????", "????????????? ????????? ?????? ?? ?????????????? ?????"], // ?
                wwc: ["???? ?????", "Share calculation & resources counter + previous & next buttons on finished world wonders (currently not deactivatable!)"],
                wwr: ["", ""],
        ...
        pl: {
            settings: {
                dsc: "DIO-Tools oferuje (miedzy innymi) poprawione widoki, nowe usmieszki,<br>opcje handlu i zmiany w wygladzie.",
                act: "Wlacz/wylacz funkcje skryptu:",
                prv: "podglad poszczególnych opcji:"
            },
            options: {
                //bir: ["Licznik birem", "Zlicza i sumuje biremy z miast"],
                ava: ["Przeglad jednostek", "Wskazuje jednostki wszystkich miast"], // ?
                sml: ["Emotki", "Dodaje dodatkowe (zielone) emotikonki"],
                str: ["Sila jednostek", "dodaje tabelki z sila jednostek w róznych miejscach gry"],
                ...
                cnt: ["Podboje", "Zlicza wsparcia/ataki w oknie podboju (tylko wlasne podboje)"],
                way: ["Predkosc wojsk", "Wyswietla dodatkowo czas jednostek dla bonusu przyspieszone ruchy wojsk"],
                sim: ["Symulator", "Dostosowanie wygladu symulatora oraz dodanie szybkich pól wyboru"],
                spl: ["Ramka czarów", "Kompaktowa plywajaca ramka z czarami (mozna umiescic w dowolnym miejscu ekranu. Zapamietuje polozenie.)"],
                act: ["Ramki aktywnosci", "Ulepszony podglad ruchów wojsk i handlu (mozna umiescic w dowolnym miejscu ekranu. Zapamietuje polozenie.)"],
                pop: ["Laski", "Zmienia wyglad ramki informacyjnej o ilosci produkowanych lask"],
                tsk: ["Pasek skrótów", "Powieksza pasek skrótów i minimalizuje okienko z bonusem dziennym"],
                // cha: ["Czat", 'Zastepuje standardowy Chat chatem IRC'],
                ...
                tic: ["Ikony miasta", "Kazde miasto otrzyma ikone typu miasta (automatyczne wykrywanie)", "Dodatkowe ikony sa dostepne dla recznego wyboru"], // ?
                til: ["Lista miasto", "Dodaje ikony miasta do listy miasta"], // ?
                tim: ["Mapa", "Zestawy ikon miasta na mapie strategicznej"], // ?
                wwc: ["Cuda Swiata", "Liczy udzial w budowie oraz ilosc wyslanych surowców na budowe Cudu Swiata oraz dodaje przyciski do szybkiego przelaczania miedzy cudami (obecnie nie mozliwe do wylaczenia)"],
                wwr: ["", ""],
                //wwi: [ "World wonder icons",'Adds world wonder icons on the strategic map' ],
                con: ["menu kontekstowe", 'Zamiemia miejcami przycisk "wybierz miasto" z przyciskiem "podglad miasta" po kliknieciu miasta na mapie'],
                sen: ["Wyslane jednostki", 'Pokaz wyslane jednostki w oknie wysylania ataków/wsparc'],
                tov: ["Podglad miasta", 'Zastepuje nowy podglad miasta starym'],
                scr: ["Zoom", 'Mozesz zmienic poziom przyblizenia mapy kólkiem myszy'],

                err: ["Automatycznie wysylac raporty o bledach", "Jesli wlaczysz te opcje, mozesz pomóc zidentyfikowac bledy"]

            },
            labels: {
                uni: "Przeglad jednostek",
                total: "Ogólny",
                available: "Dostepny",
                outer: "Na zewnatrz",
                con: "Wybierz miasto",
                ...
                oth: "Inne",
                // Defense form
                ttl: "Podglad: Obrona miasta",
                inf: "Informacje o miescie:",
                dev: "Ochylka",
                det: "jednostki ladowe",
                prm: "opcje Premium",
                sil: "Ilosc srebra",
                mov: "Ruchy wojsk",
                WaUp: "si nivel muralla ?25 :",
                ...
                spd: "Predkosc",
                bty: "Lup (surowce)",
                cap: "Pojemnosc transportu",
                res: "Koszta (surowce)",
                fav: "Laski",
                tim: "Czas rekrutacji (s)",
                // Trade
                rat: "Stosunek surowców dla wybranej jednostki",
                shr: "procent zapelnienia magazynu w docelowym miescie",
                per: "Handel procentowy",
                // Sent units box
                lab: "Wyslane jednostki",
        ...
        br: {
            ...
            options: {
                ...
                for: ["Formulario Excel", "Formulario Excel para Cerco, "],
        ...
        cz: {
            settings: {
                dsc: "DIO-Tools nabízí,mimo jiné,nekterá nová zobrazení,okénko smajlíku,<br>obchodní možnosti a nekteré zmeny v rozložení panelu.",
                act: "Aktivovat/Deaktivovat funkce  sady nástroju:",
                prv: "Ukázka nekolika funkcí:",

                version_old: "Verze je zastaralá",
                version_new: "Verze je aktuální",
                version_dev: "Vývojárská verze",
                ...
                cat_units: "Jednotky",
                cat_icons: "Ikony mest",
                cat_forum: "Forum",
                cat_trade: "Obchod",
                cat_wonders: "Div sveta",
                cat_layout: "Okna",
                cat_other: "Ostatní"
            },
            options: {
                // bir: ["Pocítadlo birém", "Spocítá každé birémy ve mestech a secte je."],
                ava: ["Jednotky Prehled", "Oznacuje jednotky všemi mesty"], // ?
                sml: ["Smajlíci", "Rozširuje panel BBkodu okénkem smajlíku"],
                str: ["Síla jednotek", "Pridává tabulku sil jednotek v ruzných  oblastech"],
                tra: ["Transportní kapacita", "Zobrazuje obsazenou a dostupnou transportní kapacitu v nabídce jednotek"],
                per: ["Procentuální obchod", "Rozširuje obchodní okno možností procentuálního obchodu"],
                rec: ["Obchod rekrutace", "Rozširuje obchodní okno možností obchodem pro rekrutaci"],
                cnt: ["Dobývání", "Pocítá Útok/Obrana v okne dobývání (pouze vlastní dobývání zatím)"],
                way: ["Rychlost vojsk", "Zobrazuje vylepšenou rychlost vojsk v okne útoku/obrany"],
                sim: ["Simulátor", "Prizpusobení rozložení simulátoru & permanentní zobrazování rozšíreného okna modifikátoru"],
                spl: ["Okénko kouzel", "Stlacené klouzání oken & magnetické okénko kouzel (pozice pameti)"],
                act: ["Aktivní okénka", "Zlepšený zobrazení obchodu a vojsk aktivními okénky (pozice pameti)"],
                pop: ["Vyskakovací okénko prízne", "Zmení vyskakovací okno seznamu prízní"],
                tsk: ["Hlavní panel", "Zvyšuje hlavní panel a minimalizuje bonus denní odmeny po prihlášení"],
                // cha: ["Chat", 'Nahrazen aliancní chat chatem IRC.'],
                bbc: ["Obranné hlášení", "Rozširuje panel BBkodu automatickém hlášení obrany mesta"],
                com: ["Porovnání jednotek", "Pridává tabulku porovnání jednotek"],
                tic: ["Ikony mest", "Každé mesto dostává svojí ikonku dle typu mesta (automatická detekce)", "Další ikonky jsou k dispozici manuálne"],
                til: ["Seznam mest", "Pridává ikony mest do seznamu mest"],
                tim: ["Mapa", "Pridává ikony mest na stategickou mapu"],
                wwc: ["Kalkulacka", "Výpocet podílu & pocítadlo surovin  + predchozí & další tlacítka  na dokoncených divech sveta (aktuálne nelze deaktivovat!)"],
                wwr: ["Žebrícek", "Predelaný žebrícek divu sveta"],
                wwi: ["Ikony", 'Prídává ikony divu sveta na strategickou mapu'],
                con: ["Kontextové menu", 'Vymenuje "Vybrat mesto" a "Prehled mesta" v kontextovém menu'],
                sen: ["Odeslané jednotky", 'Zobrazuje odeslané jednotky útoku/obrany v okne'],
                tov: ["Prehled mesta", 'Nahrazuje nový prehled mest starším stylem okna'],
                scr: ["Kolecko myši", 'Mužeš zmenit pohledy s koleckem myši'],

                err: ["Hlásit chyby automaticky", "Pokud aktivuješ tuto možnost,pomužeš nám identifikovat chyby."],
                her: ["Thrácké dobývání", "Redukuje mapy Thráckého dobývání."]
            },
            labels: {
                uni: "Jednotky Prehled",
                total: "Celkový",
                ...
                xma: "Vánoce",
                // Defense form
                ttl: "Prehled: Obrana mesta",
                inf: "Informace o meste:",
                dev: "Odchylka",
                ...
                // Trade
                rat: "Pomer surovin typu jednotky",
                shr: "Podíl na úložné kapacite cílového mesta",
    ...
    function getLatestVersion() {
        $('<style id="dio_version">' +
            '#version_info .version_icon { background: url(https://i.imgur.com/emYBqrL.png) -50px -50px no-repeat; width:25px; height:25px; float:left; } ' +
        ...
        if (version_text === '') {
            $.ajax({
                type: "GET", url: "https://www.dropbox.com/s/th176xxk55dq365/Version.php?dl=0",//"https://diotools.de/scripts/version.php"
        ...
        $("#dio_tools").click(function () {
            ...
            if (!$('#dio_settings').get(0)) {
                ...
                $('.settings-container').append(
                    ...
                    '<td><img src="https://i.imgur.com/DUWU0L0.png" alt="" /></td>' +
                    ...
                    '<td><img src="https://i.imgur.com/615jhgK.png" alt="" /></td>' +
                    ...
                    '</td>' +
                    '</tr><tr>' +
                    '<td><img src="https://i.imgur.com/j1kFgBz.png" alt="" style="border: 1px solid rgb(158, 133, 78);" /></td>' +
                    ...
                    '</tr><tr>' +
                    '<td><img src="https://i.imgur.com/aLx2BHg.png" alt="" /></td>' +
                    ...
                    '<td><img src="https://i.imgur.com/Xo7h6n7.png" alt="" /></td>' +
                    ...
                    '<img src="https://i.imgur.com/f8WfWVa.gif" alt="" /> <img src="http://666kb.com/i/cigmv8wnffb3v0ifg.gif" /> ' +
 ...
 '<br><br><br></td>' +
                    '</tr><tr>' +
                    '<td><img src="https://i.imgur.com/v0RLfUp.png" alt="" /></td>' +
                  ...
                  //Link excel Formulario Cerco ate aqui

                    '</tr></table>' +

                        // Trade tab
                    '<table id="dio_trade_table" class="content_category"><tr>' +
                    '<td><img src="https://i.imgur.com/98zHZ89.png" /></td>' +
                    ...
                    '<p>' + getText("options", "rec")[1] + '</p><br>' +
                         ...
                         */
                    '</tr><tr>' +
                    '<td><img src="https://i.imgur.com/2IV0HWU.png" /></td>' +
                    ...
                    '<td><img src="https://i.imgur.com/QOdJ9ni.png" alt="" /></td>' +
...
// caixa dio-gatao configuraçoes
    function addSettingsButton() {
        var tooltip_str = "DIO-Gatao: " + (DM.getl10n("layout", "config_buttons").settings || "Settings");
        ...
        $('<style id="dio_settings_button" type="text/css">' +
            ...
            '#ui_box .dio_settings .dio_icon { margin:7px 0px 0px 4px; width:24px; height:24px; background:url(https://grmh.pl//e/g/Animals/0052.gif) no-repeat 0px 0px; background-size:100% } ' +
    ...
    function setStyle() {
       ...
       // http://s14.directupload.net/images/140826/mh8k8nyw.jpg

         // details tropas enviados divinos dos deus devem ser selesionada com medidas exemplo -210px -210px no-repeat;
       // $('<style id="dio_godsent_details" type="text/css"> #godsent_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
       // $('<style id="dio_godsent_details" type="text/css"> #harpy_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
       // $('<style id="dio_godsent_details" type="text/css"> #medusa_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
        //$('<style id="dio_godsent_details" type="text/css"> #manticore_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
        //$('<style id="dio_godsent_details" type="text/css"> #minotaur_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
        //$('<style id="dio_godsent_details" type="text/css"> #zyklop_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
       // $('<style id="dio_godsent_details" type="text/css"> #sea_monster_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
       // $('<style id="dio_godsent_details" type="text/css"> #centaur_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
        //$('<style id="dio_godsent_details" type="text/css"> #pegasus_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
       // $('<style id="dio_godsent_details" type="text/css"> #cerberus_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
        //$('<style id="dio_godsent_details" type="text/css"> #fury_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
       // $('<style id="dio_godsent_details" type="text/css"> #griffin_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');
        //$('<style id="dio_godsent_details" type="text/css"> #calydonian_boar_details { background-image: url(https://i.imgur.com/gEbOLwI.png); } </style>').appendTo('head');

       // big gods
        $('<style id="dio_big_gods" type="text/css"> #temple_god_static { background-image: url(https://i.imgur.com/IU2eiH9.png); } </style>').appendTo('head');

        // god mini
        $('<style id="dio_god_mini" type="text/css"> #temple_zeus { background-image: url(https://i.imgur.com/LNptz3f.png); } </style>').appendTo('head');
        $('<style id="dio_god_mini" type="text/css"> #temple_poseidon { background-image: url(https://i.imgur.com/LNptz3f.png); } </style>').appendTo('head');
        $('<style id="dio_god_mini" type="text/css"> #temple_hera { background-image: url(https://i.imgur.com/LNptz3f.png); } </style>').appendTo('head');
        $('<style id="dio_god_mini" type="text/css"> #temple_athena { background-image: url(https://i.imgur.com/LNptz3f.png); } </style>').appendTo('head');
        $('<style id="dio_god_mini" type="text/css"> #temple_hades { background-image: url(https://i.imgur.com/LNptz3f.png); } </style>').appendTo('head');
        $('<style id="dio_god_mini" type="text/css"> #temple_artemis { background-image: url(https://i.imgur.com/LNptz3f.png); } </style>').appendTo('head');
...
//alterar a cor du texto na caixa visão geral das unidades  / cor origem 126,223,126
    ...
    var Notification = {
        init: function () {
            ...
            $('<style id="dio_notification" type="text/css">' +
                '#notification_area .diotools .icon { background: url(https://i.imgur.com/f8WfWVa.gif) 4px 7px no-repeat !important;} ' +
    ...
    var AvailableUnits = {
        activate: function () {
            ...
            $('<style id="dio_available_units_style">' +
              ...
              // imagem da visao geral das unidades (original...http://s1.directupload.net/images/140323/w4ekrw8b.png....minha....https://i.imgur.com/RlSHFrr.png....background-size:100%)
    ...
    var Simulator = {
        activate: function () {
            $('<style id="dio_simulator_style" type="text/css">' +
                ...
                '.place_simulator_table .place_image.alliance_modifier { background: url(https://i.imgur.com/iJBVgY8.png); background-position: -22px -88px; } ' +
                '.place_simulator_table .place_image.building_tower { background: url(https://i.imgur.com/iJBVgY8.png); background-position: -44px -44px; } ' +
                '.place_simulator_table .place_image.ground_factor { background: url(https://i.imgur.com/iJBVgY8.png); background-position: -22px -22px; } ' +
                '.place_simulator_table .place_image.naval_factor { background: url(https://i.imgur.com/iJBVgY8.png); background-position: -22px -44px; } ' +
                '.place_simulator_table .place_image.luck { background: url(https://i.imgur.com/iJBVgY8.png); background-position: -44px -22px; } ' +
                '.place_simulator_table .place_image.morale { background: url(https://i.imgur.com/iJBVgY8.png); background-position: 0px 44px; } ' +
                '.place_simulator_table .place_image.wall_level { background: url(https://i.imgur.com/iJBVgY8.png); background-position: -66px -66px; } ' +

                '.place_simulator_table .place_image.is_night { background: url(https://i.imgur.com/iJBVgY8.png); background-position: -44px -88px; } ' +
                '.place_simulator_table .place_image.research_ram { background: url(https://i.imgur.com/iJBVgY8.png); background-position: -44px -66px; } ' +
                '.place_simulator_table .place_image.research_phalanx { background: url(https://i.imgur.com/iJBVgY8.png); background-position: -22px -66px; }' +
                '.place_simulator_table .place_image.research_divine_selection { background: url(https://i.imgur.com/iJBVgY8.png); background-position: 0px 22px; }' +
                '.place_simulator_table .place_image.research_combat_experience { background: url(https://i.imgur.com/iJBVgY8.png); background-position: 0px 66px; }' +
    ...
    function addForm(e) {
        ...
        $(bbcodeBarId + ' .dio_bbcode_option').css({
            background: 'url("http://i.imgur.com/Cho3XXa.png")',
            display: 'block',
            float: 'left',
            width: '22px',
            height: '23px',
        ...
        var imgArray = {
			OC: 'http://i.imgur.com/wr54ZjX.png',
            SOS: 'http://i.imgur.com/77prwjs.png',
            Bir1: 'http://wiki.en.grepolis.com/images/4/44/Bireme_40x40.png',
			Def1: 'http://s7.directupload.net/images/140115/ty6szerx.png',
            Rev2: 'http://images.uncyc.org/pt/thumb/6/6e/Akuma-flame.gif/15px-Akuma-flame.gif',
            WallUp: 'http://www.chez.com/lesoutilsduwebmestre/fleches/4da/097ha.gif',
            ...
            bordure: 'http://i.imgur.com/5RCRyzj.jpg'
        };
        ...
        $('<div class="bb_def_chooser">' +
           ...
           //  nao aparecer '<div id="f_uni" class="checkbox_new checked"><div class="cbx_icon"></div><div class="cbx_caption">' + getText("labels", "det") + '</div></div><br><br>' +
		    ...
		    '<div id="f_R22" class="checkbox_new checked"><div class="cbx_icon"></div><div class="cbx_caption">' + getText("labels", "R22") + '</div></div><br><br>' +
        ...
        $(bbcodeBarId + ' #dio_insert').click(function () {
            ...
            var str = '[center]' + '[font=serif]' + '\n\n[color=#07701B][size=22][u][b]' + getText("labels", "ttl") + ' [/b][/u][/size][/color]\n\n' + '[/font]' + '[/center]' +
                ...
                '[size=12][town]' + uw.ITowns.getTown(uw.Game.townId).getId() + '[/town] ([player]' + uw.Game.player_name + '[/player]) [/size]' + '[/center]' +
                '\n\n[color=#28045A][size=12][i][b]' + getText("labels", "inf") + '[/b][/i][/size][/color]' +
                '[table][*]' +
                ...
                ($('#f_sil').hasClass("checked") ? '[center]' + '0 k[/center][||]' : ' ') +
                '[center]' + uw.ITowns.getTown(uw.Game.townId).getAvailablePopulation() + '[/center][||]' +
                '[center]' + '[b][color=#004d00]✔️[/color][/b][/center][||]' +
                '[center]' + ($('#f_Bir1').hasClass("checked") ? '[img]' + imgArray.SOS + '[/img]' + '[/center][||]' : ' ') +
                '[center]' + ($('#f_Def1').hasClass("checked") ? '[img]' + imgArray.SOS + '[/img]' + '[/center][||]' : ' ') +
                ($('#f_OC').hasClass("checked") ? '[center]' + '🔱🔱[/center]' : ' ') +
                '[/**][/table]' +
            ...
            str += move_table + '[center]' + '[img]' + imgArray.bordure + '[/img]' + '[center]';
    ...
    var SmileyBox = {
        ...
        add: function (e) {
            ...
            $('<img class="smiley_button" src="https://i.imgur.com/bW8lxNz.gif">').appendTo(bbcodeBarId + ' .bb_button_wrapper');
    ...
    var FavorPopup = {
        godArray: {
            ...
            hades: '-203px',
            artemis: '-305px'
        }, godImg: (new Image()).src = "https://i.imgur.com/cPIxRst.png",
    ...
    function addForm(e) {
        ...
        $(bbcodeBarId + ' #dio_insert').click(function () {
            ...
            var str = '[center]' + '[font=serif]' + '\n\n[color=#07701B][size=22][u][b]' + getText("labels", "ttl") + ' [/b][/u][/size][/color]\n\n' + '[/font]' + '[/center]' +
                ...
                '[**][center]' + ($('#f_WaUp').hasClass("checked") ? '[img]' + imgArray.WallUp + '[/img]': ' ') + uw.ITowns.getTown(uw.Game.townId).buildings().getBuildingLevel("wall") + '[/center][||]' +
                '[center]' + uw.ITowns.getTown(uw.Game.townId).buildings().getBuildingLevel("tower") + '[/center][||]' +
                '[center]' + (uw.ITowns.getTown(uw.Game.townId).researches().attributes.phalanx ? '[b][color=#004d00]?[/color][/b]' : '[b][color=#992600]?[/color][/b]') + '[/center][||]' +
                '[center]' + (uw.ITowns.getTown(uw.Game.townId).researches().attributes.ram ? '[b][color=#004d00]?[/color][/b]' : '[b][color=#992600]?[/color][/b]') + '[/center][||]' +
                ($('#f_prm').hasClass("checked") ? '[center]' + ((uw.Game.premium_features.commander >= uw.Timestamp.now()) ? '[b][color=#004d00]?[/color][/b]' : '[b][color=#992600]?[/color][/b]') + '[/center][||]' : ' ') +
                ($('#f_prm').hasClass("checked") ? '[center]' + ((uw.Game.premium_features.captain >= uw.Timestamp.now()) ? '[b][color=#004d00]?[/color][/b]' : '[b][color=#992600]?[/color][/b]') + '[/center][||]' : ' ') +
                ($('#f_prm').hasClass("checked") ? '[center]' + ((uw.Game.premium_features.priest >= uw.Timestamp.now()) ? '[b][color=#004d00]?[/color][/b]' : '[b][color=#992600]?[/color][/b]') + '[/center][||]' : ' ') +
                ($('#f_sil').hasClass("checked") ? '[center]' + '0 k[/center][||]' : ' ') +
                '[center]' + uw.ITowns.getTown(uw.Game.townId).getAvailablePopulation() + '[/center][||]' +
                '[center]' + '[b][color=#004d00]?[/color][/b][/center][||]' +
                '[center]' + ($('#f_Bir1').hasClass("checked") ? '[img]' + imgArray.SOS + '[/img]' + '[/center][||]' : ' ') +
                '[center]' + ($('#f_Def1').hasClass("checked") ? '[img]' + imgArray.SOS + '[/img]' + '[/center][||]' : ' ') +
                ($('#f_OC').hasClass("checked") ? '[center]' + '??[/center]' : ' ') +
                '[/**][/table]' + 
